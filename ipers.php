...
//SELECT * FROM  `invent` WHERE  `pl_id` =  '21234879'
...
if($_GET['Ajax'] == 'yes'){
	if(strlen($_GET['q'])>0){
		$GetUsers = mysqli_query($GLOBALS['db_link'],"SELECT * FROM `user` WHERE `login` LIKE '".preg_replace("/[^a-zA-Zа-яА-Я0-9 _-]/","",iconv("UTF-8","Windows-1251",$_GET['q']))."%' ORDER BY `level` DESC LIMIT 7;");
...
if($_POST['newnick']){echo "<script type='text/javascript'> parent.location.href = '?".mysqli_real_escape_string($_POST['newnick'])."'; </script>";}
if(!empty($_GET['info'])){
...
<head> 
<title>".$pers['login']." - информация персонажа бесплатной браузерной онлайн игры .</title> 
<meta http-equiv=\"Content-Type\" content=\"text/html; charset=windows-1251\"/> 
<meta http-equiv=\"pragma\" Content=\"no-cache\"> 
<meta http-equiv=\"cache-control\" content=\"no-cache, no-store\"> 
<meta http-equiv=\"expires\" content=\"0\"> 
<meta name=\"description\" content=\"Информация персонажа бесплатной ролевой онлайн игры « ». Огромный фэнтезийный мир!\"> 
...
<body>";
if(!empty($_GET['admin_move']) and ($WatchUser['login']=='Vellini' or $WatchUser['login']=='Инквизитор' or $WatchUser['login']=='')){
...
parent.window.opener.parent.frames[\'ch_list\'].location = \'/ch.php?lo=1\';
    ...
    mysqli_query($GLOBALS['db_link'],"UPDATE `user` SET `loc`='".$pers['loc']."',`pos`='".$pers['pos']."' WHERE `id`='".$WatchUser['id']."'");
}
echo'
<style>
.transp{
...
$viplist = array('Vellini');
	...
	echo "	
	background: url('/images/gold_money.png') top left no-repeat fixed, url('/image/back_top.jpg') top center repeat-x, url('/image/back.jpg');
	";
	}
echo'
}
...
';
$ViewUser = explode("|", $pers['n_obraz']);
$UserSex = (($pers['sex'] == 'male') ? 'm':'f');
$ShowUser = '/images/flash/avatars/'.$UserSex.$ViewUser[0].'.png|0|0|';
if($UserSex == 'm')
	$ShowUser .= '/images/flash/avatars/b/brown_000'.$ViewUser[1].'.png|67|27|';
$ShowUser .= '/images/flash/avatars/h/'.$UserSex.'_brown_000'.$ViewUser[2].'.png|'.(($UserSex == 'm')? '60' : '44').'|0|';

//$ShowUser .= '/images/flash/items/weapon000021'.$UserSex.'.swf|0|0|';
//$ShowUser .= '/images/flash/items/bone40shield000002'.$UserSex.'.swf|0|0|';
...
var ViewUser = '/images/flash/avatars/reg_bg.jpg|0|0|".$ShowUser."';
var presents = [";
$presents = mysqli_query($GLOBALS['db_link'],"SELECT * FROM `podarki` WHERE `id`='".$pers['id']."';");
$present = '';
...
var params = [['".$pers['login']."',".$pers['sklon'].",'".(($pers['login']=='Vellini' or $pers['login']=='Vellini' or $pers['login']=='Vellinitu' or $pers['clan_id']=='none')?'none':$pers['clan_gif'])."','".$pers['level']."','".$pers['obraz']."','".$location['city']."@".(($location['room'])?$location['room']:$location['loc']).(($WatchUser['login'] == 'Vellini' || $WatchUser['login'] == 'Vellini' || $WatchUser['login'] == 'Vellinitu')?'<br />(<a href="?p='.$_GET["p"].'&no_watch=yes&admin_move=yes"><font color="red">'.$pers['x'].':'.$pers['y'].'</font></a>)':'')."',".(($pers['last']<time()-300)?'0':(($pers['invisible']>time())?'0':'1')).",".$pers['battle'].",'".(($pers['clan_id']=='chaos')?'':$pers['clan'])."','".$pers['clan_d']."','Барус','".date("d.m.Y",$pers['bdaypers'])."','".$pers['semija']."'],[['Мощь',".($perk['7']?$pers['sila']+2:$pers['sila']).",".$nst['30']."],['Проворность',".($perk['9']?$pers['lovk']+2:$pers['lovk']).",".$nst['31']."],['Везение',".($perk['10']?$pers['uda4a']+2:$pers['uda4a']).",".$nst['32']."],['Разум',".($perk['11']?$pers['znan']+2:$pers['znan']).",".$nst['34']."],['Здоровье',".($perk['8']?$pers['zdorov']+2:$pers['zdorov']).",".$nst['33']."],['Сноровка',".$pers['mudr'].",0]],[['Класс брони',".($perk['32']?$nst['9']+30:$nst['9'])."],['Уловка',".($perk['19']?$nst['5']+30:$nst['5'])."],['Точность',".($perk['0']?$nst['6']+30:$nst['6'])."],['Сокрушение',".($perk['5']?$nst['7']+30:$nst['7'])."],['Стойкость',".($perk['15']?$nst['8']+30:$nst['8'])."],['Пробой брони',".$nst['10']."]]];\n";
$slot= mysqli_query($GLOBALS['db_link'],"SELECT `invent`.*, `items`.* FROM `items` INNER JOIN `invent` ON `items`.`id` = `invent`.`protype` WHERE `pl_id`='".$pers['id']."' and `used`='1' ORDER BY `invent`.`curslot`;");
...
echo "var warstats = ['".$wins[0]."','".$wins[1]."','".$wins[2]."','".$wins[3]."','".mysqli_num_rows(mysqli_query($GLOBALS['db_link'],"SELECT * FROM `quest_completed` WHERE `que_st`='1' AND `usr_id`='".$pers['id']."'"))."'];";
//
//опыт
$exp=explode("|",$pers['exp']);
$xyz=array_sum($exp);
echo "vzlomshik_exp = ['".htmlspecialchars($exp[2])."'];";
echo "var exp = ['".$xyz."','".$exp[0]."','".$exp[1]."','".$exp[2]."'];";
...
echo"];\n";
if(accesses($WatchUser['id'],'pvu')){
	if($pers['login']=='Vellini' or $pers['login']=='Vellini' or $pers['login']=='Vellinitu' or $pers['sign']=='kgTvx2WrEZ'){
		$pers['ip']='0.0.0.0';
		$pers['lastip']='0.0.0.0';	
//(($pers['sex'] == 'male' and $pers['clan']!='Vellini' and $pers['type']!='3' and $pers['login']!='Генералиссимус')?'0':'1')	ы	
	}
	if($WatchUser['login']=='Vellini' or $WatchUser['login']=='Vellini' or $WatchUser['login']=='Vellinitu' or $WatchUser['clan_id']=='1'){
		echo"var info = ['".$pers['name']."','".$pers['country']."','".$pers['city']."',".(($pers['sex'] == 'male')?'0':'1').",'".$pers['url']."','<a href=mailto:".$pers['email'].">".$pers['email']."</a>','".$pers['bday']."','".$pers['icq']."','".$pers['id']."','".(($pers['ip']!=$pers['lastip'])?'<a href="http://ip2geolocation.com/?ip='.$pers['lastip'].'" target="_blank">'.$pers['lastip'].'</a> > <a href="http://ip2geolocation.com/?ip='.$pers['ip'].'" target="_blank">'.$pers['ip'].'</a>':'<a href="http://ip2geolocation.com/?ip='.$pers['ip'].'" target="_blank">'.$pers['ip'].'</a>')."','".date("d.m.Y H:i:s",$pers['last']?$pers['last']:0)."',[".$pers['id'].",0,'123'],'".round($pers['nv'], 2)." LR','".round($pers['dd'], 2)." DLR','".round($pers['baks'], 2)." $',1];\n";
	}
	else{
...
</HEAD><body bgcolor=#ffffff topmargin=0 bottommargin=0 marginwidth=0 marginheight=0 leftmargin=0 rightmargin=0 link=#336699 alink=#336699 vlink=#336699><table cellpadding=4 cellspacing=0 border=0 width=100%><tr><td bgcolor=#FCFAF3 align=center><font class=nickname><b>Панель Гвардии Неприкасаемых</b>'.($_GET['popup']?'':' <a href="javascript:ShowHide_wtch()" id="wtch">[Показать]</a>').'</font></td></tr></table><table cellpadding=0 cellspacing=0 border=0 width=100%><tr><td bgcolor=#Ffffff><img src=/image/1x1.gif width=1 height=1></td></tr><tr><td bgcolor=#B9A05C><img src=/image/1x1.gif width=1 height=1></td></tr><tr><td bgcolor=#F3ECD7><img src=/image/1x1.gif width=1 height=2></td></tr></table>
<div align="center">';
